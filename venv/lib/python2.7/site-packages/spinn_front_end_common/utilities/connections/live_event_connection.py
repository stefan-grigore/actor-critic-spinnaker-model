...
class LiveEventConnection(DatabaseConnection):
    ...
    def add_start_callback(self, label, start_callback, index):
        ...
        :param start_callback: A function to be called when the start\
                    ...
                    label of the referenced vertex, an instance of\
                    this class, which can be used to send events,
                    and the index of the neuron to be fired
        ...
        logger.warning(
            ...
            "future release.")
        self.add_start_resume_callback(label, start_callback, index)

    def add_start_resume_callback(self, label, start_resume_callback, index):
        self._start_resume_callbacks[label].append((start_resume_callback, index))

    def clear_start_resume_callbacks(self, label):
        while len(self._start_resume_callbacks[label]) != 0:
            self._start_resume_callbacks[label].pop()
